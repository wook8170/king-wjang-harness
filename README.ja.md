...
| 指標 | 値 |
|---|---|
| フックレイテンシ (p95) | **< 150 ms**（実測 62 ms・10万件ジャーナル再生フォールバックで 102 ms） |
| テストスイート | **893件 パス**（33ファイル） |
...
**v0 —— コアエンジン・ゲート・構築/出荷トラックまで実装・実測済み**（テスト893件）。ただし
...
- ゲートはフィラー文書を通す —— 内容の**質**の判定は決定的なローカルコアの外であり、人間の承認が防御。
- 人が `.harness/events.jsonl` を手で書き換えるのは **脅威モデルの外** —— フックはエージェントを止めるが、持ち主は止めない。
- フックは **解決できる範囲まで** 見る —— `sh -c`、深さ3までのスクリプト、`npm run` のスクリプト。**`make <target>` は解決しない**（Makefile の構文解析は範囲外）。深さ4以上のスクリプト連鎖も追わない。
- **ジャーナルの圧縮コマンドは意図的に置かない。** `events.jsonl` は監査記録であり、それを書き換えるコマンドは **何も消してはならない場所での削除プリミティブ** になる。無いことの代償は限定的だ —— 10万件（数十年分）の再生で p95 ≈ 101ms、しかも状態が劣化している間だけで、`doctor --repair` で終わる。
