...
| テストスイート | **1548 tests**（66ファイル）— **配布物では1524件がパス**（残りは配布物から除外されるリポジトリ内部文書を見る検査、`git archive` 実測）。時間の断定2件は、忙しいマシンでは判定の代わりに**「測定不能」とその時の負荷**を記録する |
...
（1548 tests）、その上でハーネス**自身**のリリース準備ゲートを通した。出荷規律を強制する道具を、
その規律で測って通した。
...
- **1548 tests green · `tsc` 0 · 繰り返し実行しても決定的。**
