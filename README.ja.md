...
| テストスイート | **1506 tests**（64ファイル）— **配布物では1484件がパス**（残りは配布物から除外されるリポジトリ内部文書を見る検査、`git archive` 実測）。時間の断定2件は、忙しいマシンでは判定の代わりに**「測定不能」とその時の負荷**を記録する |
...
（1506 tests）、その上でハーネス**自身**のリリース準備ゲートを通した。出荷規律を強制する道具を、
その規律で測って通した。
...
- **出荷ゲート13個 → 12個が実測 PASS**（13個目のフック遅延はアイドルなマシンで綺麗に再測定される）:
  ...
  シークレット履歴（gitleaks、コミット307件: **0**）· CLI 契約 · 可観測性 · パッケージング。
...
- **1506 tests green · `tsc` 0 · 繰り返し実行しても決定的。**
