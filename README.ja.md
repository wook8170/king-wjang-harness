...
| キー | 既定値 | 役割 |
|---|---|---|
| `lang` | `en` | ハーネスが出力するすべての文言の言語 — CLI・フック JSON・MCP・生成ドキュメント。日本語環境では `ko`（韓国語）も選べる。 |
...
| `design_allowed_prefixes` | `['.harness/', 'docs/']` | **設計トラックで書き込みが許可される場所。** この接頭辞の外は P6 ゲート承認まで拒否される。 |
...
**v0 —— コアエンジン・ゲート・構築/出荷トラックまで実装・実測済み**（テスト1073件）。ただし
