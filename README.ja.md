...
| テストスイート | **1490件 パス**（63ファイル）— うち17件はリポジトリ専用の検査で、配布物ではskipされる（配布物では1450件） |
...
（1490 tests）、その上でハーネス**自身**のリリース準備ゲートを通した。出荷規律を強制する道具を、
その規律で測って通した。
...
- **1490 tests green · `tsc` 0 · 繰り返し実行しても決定的。**
