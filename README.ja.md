...
<sub>フック + CLI ・ 追記専用イベントジャーナルを信頼できる情報源として使用 ・ セッションあたりコンテキスト ~260 トークン ・ オプトインしていないプロジェクトでは無音</sub>
...
| **コンテキストコスト** | ルールを追加するたびに増大する | **セッションあたり ~260 トークンで一定** —— ルールはプロセス外で動くので、増やしても増えない |
...
>
> `king-wjang-harness` のドライバースキルは、**これらのコンパニオンを点検し、無いものをインストールする** —— ネットワーク動作であり、通常の Bash 権限層を通し、既知のマーケットプレイスに限定される。必須ではない: 無くてもフックはプロセスを強制する。
...
### Claude Design 連携 — キャンバス取得は P4 スキルが担う

*（この節の見出しは元は「ロードマップ」だった。静かな方向に間違っていた —— 以下すべては実装・実測済みである。キャンバス取得はコアには無い —— 強制フックをローカルに保つための«意図した»設計だ —— 代わりに P4 スキルが WebFetch で取得し、本文をコアに渡す。過小広告もまた文書の欠陥だ。ただ誰も文句を言わないだけである。）*

- **キャンバス = 視覚的な信頼できる情報源。** 1つのアートボード ↔ 1つのUXノード（`"UX-7 Checkout"`）。キャンバスのURLは設計台帳に保持される。 **コア**はキャンバスをネットワーク経由で取得しない（強制フックをローカルかつ高速に保つため）。代わりに **P4スキル**が行う: WebFetch でキャンバスを取得し、その本文を `harness design sync <UX-x> --from <file>` に一つの流れで渡す —— エージェントがキャンバスを引き、コアはローカルのままだ。
...
- **非干渉** —— `.harness/` ディレクトリが*ない*プロジェクトでは、すべてのフックが沈黙を返す —— 出力もファイルもコンテキストトークンも無い。かかるのは**時間**で、シェルゲートが Node 起動前に **p95 約4ms** で抜ける。グローバルにインストールしてもリスクはゼロ。**プロジェクト単位**で、`harness init` の後にのみ有効化される。
...
- **ランタイム自己完結** —— ビルド済みの `core/dist/` はコミットされており、素の clone だけでビルドステップなしに動作する。`yaml` はインラインでバンドルされ（その ISC 表示はバンドル«内»に同梱される）、ランタイム入口の外部参照は Node 組み込みのみ。`npm audit --omit=dev`: **脆弱性0件。**（範囲: `claude plugin install` はプラグインキャッシュ内で `npm ci --ignore-scripts` を実行する。本リポジトリは `omit=dev` を書いた `.npmrc` を同梱するため、開発ツールチェーンはもう降りてこない —— 実測でバージョンごとに **1.3MB・10パッケージ**（従来は約81MB）。配布物のテストを自分で実行するには `npm install --include=dev` で明示的に有効化する。）

### 実測値

| 指標 | 値 |
|---|---|
| フックレイテンシ (p95) | 二つの表面があり、**どちらも `npm run bench:hook` が出力する**。*インプロセス*（バンドル読込済みでの判定そのもの）: 通常 **0.9 ms**、10万件（15 MB）ジャーナル再生フォールバック中は **18.6 ms**。*実時間*（ツール呼び出しが実際に待つ値）は同じ実行で **77 ms** / **102 ms** —— そのうち **40 ms がそのマシンの `node` 起動**である。実時間の絶対値はマシンの性質であり、ゲートはフォールバックが**加える**値に掛かる（インプロセス +17.7 ms・実時間 +24.7 ms・閾値 50 ms）。 |
| テストスイート | **1596 tests**（68ファイル）— **配布物では1572件がパス**（残りは配布物から除外されるリポジトリ内部文書を見る検査、`git archive` 実測）。時間の断定2件は、忙しいマシンでは判定の代わりに**「測定不能」とその時の負荷**を記録する |
| セッションあたりの追加コンテキスト | ハーネス有効時 **~260トークン**・`.harness/` の無いプロジェクトでは **0**。再測定: `echo '{"hook_event_name":"SessionStart"}' \| ./bin/harness-hook session-start \| wc -c` —— 注入されるブリーフィングは ~1KB で（プロジェクトパスを含むため正確な大きさは前後する）、4 で割るとトークン概算になる |
| ランタイム依存 | **1件**（`yaml`、バンドル済み） |
| 決定性 | 3回の実行で判定が完全一致 |

**自分で測り直せる** —— `npm run bench:hook` がパッケージに同梱されている。10万件のジャーナルを
3つの形（現実・破損・敵対的）で合成し、**両方の表面**——インプロセスの判定と実際のフックプロセス——を
計測して、**この機械の `node` 起動の下限値**も併記する —— 絶対値の大部分はその下限であって、このツールではないからだ。
...
```bash
npm install --include=dev   # .npmrc が dev を除外するため明示する。prepare hook が core/dist を構築
./bin/harness --version
```
</details>
...
**`harness` はどこにあるか。** CLI はプラグインの中に同梱されている。Claude Code がプラグインの
`bin/` を**エージェントがコマンドを実行するシェルの PATH** に載せるので、そのシェルでは下の表が
書かれたとおりに動作する —— これらのコマンドは通常そこで実行される。あなた自身のターミナルには
その PATH エントリは無い。手で動かす場合は、インストール済みプラグインの `bin/` を自分で通す:
...
| `harness gate submit <P0..P12>` | そのフェーズの成果物をレビューに出す |
| `harness gate approve <P0..P12>` | **人間のみ** —— 次のフェーズを開く承認（許可リストに入れないこと） |
| `harness report denials [--limit <n>]` | フックが最近拒否したツール呼び出し —— 何が、なぜ止まったか |
| `harness loop critical raise --reason <r> [--detail <d>]` · `harness loop critical clear` | 人間を呼び、ウェーブループを止める。判断が済んだら解除する |
...
**外し方。** `harness uninstall` は**無い** —— 意図的だ。エージェントが自分に課されたルールを
自分で切れてはならない。解除は人間が自分のターミナルで行う操作である。

```bash
rm -rf .harness/          # このプロジェクトだけ —— フックは即座に無音になる
```

イベントジャーナルも一緒に消える。ジャーナルは監査証跡なので、残したければ先に控えを取ること。
すべてのプロジェクトで止めるならプラグインを削除する（`claude plugin uninstall king-wjang-harness`）。
...
**v0.1.2 —— SHIP-READY。** コアエンジン、3つのトラック、13フェーズのすべてが実装され実測されており
（1596 tests）、その上でハーネス**自身**のリリース準備ゲートを通した。出荷規律を強制する道具を、
その規律で測って通した。

### どこまで検証したか

`verifying-production-readiness` 監査を、このリポジトリに対して**新しい独立したコンテキスト**で
実行した —— 監査者は実装者ではない —— ドキュメントを読むのではなく**製品を端から端まで実際に
動かす** **11軸**の点検である。出てきたもの:

- **出荷ゲート13個 → 12個が実測 PASS**（13個目のフック遅延はアイドルなマシンで綺麗に再測定される）:
  テスト · 型 · 自己完結クローン · フックの無害性 · 強制力 · MCP の**安全性質**（端末の前に人が
  いなければゲートは承認され得ない）· 決定性 · サプライチェーン（`npm audit --omit=dev`: **0**）·
  シークレット履歴（gitleaks、コミット307件: **0** —— これは gitleaks のルールが捕まえる範囲であり、GitHub のプッシュ保護が止める範囲とは異なる。本監査中、同一の文字列を両者が別々に判定した実例がある）· CLI 契約 · 可観測性 · パッケージング。
- **フック回避の敵対的スイープ —— 常設の300項目超コーパスの*外側*にある新しい記法を約40個**、
  すべて実際のフックに通して判定した: 新しいソース書き込みの形（`printf` · `dd` · `ex` ·
  `install` · `python3 -c` · `awk -i inplace` …）、コア / ポリシー / ジャーナルへの書き込み、
  シンボリックリンクとハードリンクの TOCTOU、ジャーナル偽造、`base64 -d | sh`、
  ネスト・配列の MCP 引数。**取り返しのつかない対象 —— コア・ポリシー・状態・イベント
  ジャーナル —— はすべて拒否された。「止めてはいけない」対照群での過剰ブロックは0件。**
- **1596 tests green · `tsc` 0 · 繰り返し実行しても決定的。**

**判定: SHIP-READY —— 出荷を止める欠陥なし。** 意図的に開けてある透過性が一つあり、それは隠さず
書いてある: 設計 → ソースの分離は*セキュリティの壁ではなく速度制限バンプ*だ（「既知の限界」と FAQ
を参照）。漏れたら取り返しのつかないもの —— ジャーナル・ポリシーファイル・コア —— は壁である。

> この監査は、このプラグインが伴走規律として同時に差し出すものそのものだ。強制することを自ら実践
> している —— 上の判定は、そのゲートをハーネス自身に回して出た結果である。
...
- ✅ **フェーズスキル —— 13フェーズすべて**（P0–P12: 設計 `P0–P6`・構築 `P7–P9`・出荷 `P10–P12`）+ researcher / design-auditor / wave-executor / wave-verifier / readiness-auditor エージェント + ADR（`adr`）
...
- **任意の MCP サーバのスキーマをすべて知ることはできない。** 名前が書き込みを意味するツール（`write`・`edit`・`put` など）は判定し、パスらしき引数はすべて検査するが、ツール名も引数名もそう付けていないサーバはフックが見える範囲の外だ。
- **外から入ってきた内容は、コマンド文が名指ししないエイリアスを含みうる。** アーカイブ内のシンボリックリンクは、判定時点のコマンド文にもファイルシステムにも存在しないため、フックには原理的に見えない。展開・clone・インストール後に**新しく生まれるパス**への書き込みを拒否していた規則は**取り消した** —— 単語ひとつで無効化され（`mkdir h && tar -xf a.tar -C h && echo x > h/f`）、同時にごく普通の単一コマンド（`git clone <url> y && echo x > y/f`）を塞いでいた。これを閉じるのはフックではなくファイルシステム層の仕事だ。
- **既に存在するハードリンクはコアファイルについてのみ検査する。** 保護ファイルの新しい名前を作ることは拒否され、`config.yaml`・`state.json`・`events.jsonl` の既存エイリアスは inode で捕まえる。ハーネス導入前に作られた**ソースファイル**のエイリアスは捕まえない —— 全ソースを inode で照合するには書き込みのたびにディレクトリを走査せねばならず、遅い判定はタイムアウトするフックであり、タイムアウトしたフックは通してしまう。

- `verifying-production-readiness` スキルは **呼ぶが同梱していない** —— 別途インストールが必要:
  `claude plugin marketplace add wook8170/verifying-production-readiness` の次に
...
- **インタプリタに«ファイル»として渡されたプログラムも、シェルスクリプトと同じように読む** — `sed -f prog.sed`・`awk -f prog.awk`・`perl x.pl`・`python3 x.py`・`node x.js`・`bun`・`deno run`・`ruby`・`php`・`tclsh`・`lua`・`Rscript` のようにプログラムをファイルで渡すと、フックはそのファイルを読み、ハーネス管理下のパスに書き込むなら拒否する。**3つの限界は意図的だ**: ① プログラムファイルが **64 KB を超えると拒否ではなくスキップされる**（実際のバンドルは大きく、手書きの偽造は小さい —— 64 KB 超の偽造は `node dist/cli.js` を塞がないために開示して支払うコストだ）。② **フックが知らないインタプリタ**（`julia`・`groovy`・`raku` のような特殊ランタイム）は列挙の外にある。③ **パスをリテラルで書かず言語内で組み立てるプログラム** —— 文字列連結（`".har" + "ness/…"`）・`chr()`/`String.fromCharCode`・base64 —— はリテラルパス検査に掛からない（保護ディレクトリへ入る相対 `chdir(".harness")` は掛かる）。インタプリタのロングテールと言語内難読化を完全に閉じるには、フックが本文を読むのではなく**ファイルシステム層での強制**が必要で、それはフックの範囲外だ。
- **ジャーナルの圧縮コマンドは意図的に置かない。** `events.jsonl` は監査記録であり、それを書き換えるコマンドは **何も消してはならない場所での削除プリミティブ** になる。無いことの代償は限定的だ —— 10万件・15 MB（数十年分）の再生が通常経路に加える値が p95 +17.7 ms（インプロセス。実時間では +24.7 ms）、しかも状態が劣化している間だけで、`doctor --repair` で終わる。**v0.2 が加えたのは圧縮ではなく上限だ**: ジャーナルは 128 MB、設計台帳と指示書は 16 MB を超えると読み取りを拒否し、「自分で退避せよ」という処方を出す。そして `doctor` が上限の半分で先に警告する —— 限界は到達する前に見え、監査記録を代わりに消す者は依然としていない。
...
**バグ報告。** `package.json` にリポジトリと `bugs` の URL がある —— GitHub に issue を開いて
ほしい。`harness doctor` の出力と `harness --version` を添えること（どちらもファイル内容を含まない
のでそのまま貼って安全）。
