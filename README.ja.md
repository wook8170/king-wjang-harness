...
- **ランタイム自己完結** —— ビルド済みの `core/dist/` はコミットされており、素の clone だけでビルドステップなしに動作する。`yaml` はインラインでバンドルされ、ランタイム入口の外部参照は Node 組み込みのみ。`npm audit --omit=dev`: **脆弱性0件。**（範囲: `claude plugin install` はプラグインキャッシュ内で `npm ci --ignore-scripts` を実行するため、開発ツールチェーンはそこのディスクには降りてくる —— バージョンごとに73パッケージ・約66MB。そのいずれも実行経路には無い。CLI・フック・MCP サーバーは `node_modules` が無くても動作する。）
...
| テストスイート | **1494件 パス**（63ファイル）— うち20件はリポジトリ専用の検査で、配布物では実行されない（**配布物では1474件**、`git archive` 実測） |
...
（1494 tests）、その上でハーネス**自身**のリリース準備ゲートを通した。出荷規律を強制する道具を、
その規律で測って通した。
...
- **1494 tests green · `tsc` 0 · 繰り返し実行しても決定的。**
