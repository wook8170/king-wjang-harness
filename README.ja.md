...
| テストスイート | **1558 tests**（67ファイル）— **配布物では1534件がパス**（残りは配布物から除外されるリポジトリ内部文書を見る検査、`git archive` 実測）。時間の断定2件は、忙しいマシンでは判定の代わりに**「測定不能」とその時の負荷**を記録する |
...
（1558 tests）、その上でハーネス**自身**のリリース準備ゲートを通した。出荷規律を強制する道具を、
その規律で測って通した。
...
- **1558 tests green · `tsc` 0 · 繰り返し実行しても決定的。**
...
- **ジャーナルの圧縮コマンドは意図的に置かない。** `events.jsonl` は監査記録であり、それを書き換えるコマンドは **何も消してはならない場所での削除プリミティブ** になる。無いことの代償は限定的だ —— 10万件・15 MB（数十年分）の再生が通常経路に加える値が p95 +17.7 ms（インプロセス。実時間では +24.7 ms）、しかも状態が劣化している間だけで、`doctor --repair` で終わる。
