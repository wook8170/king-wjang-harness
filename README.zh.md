...
- **运行时自包含** —— 构建产物 `core/dist/` 已被提交；直接克隆即可使用，无需任何构建步骤。`yaml` 被内联打包在内，运行时入口的外部引用只有 Node 内置模块。`npm audit --omit=dev`：**0 个漏洞。**（范围说明：`claude plugin install` 仍会在插件缓存目录内执行 `npm ci --ignore-scripts`，因此开发工具链确实会落到那里的磁盘上 —— 每个版本 73 个包、约 66 MB。其中没有任何一个位于执行路径上；CLI、钩子与 MCP 服务器在没有 `node_modules` 时同样正常工作。）
...
| 测试套件 | **1494 项通过**（63 个文件）—— 其中 20 项是仅限仓库的检查，在发布包中不会运行（**发布包为 1474 项**，`git archive` 实测） |
...
（1494 tests），并在此之上通过了本工具**自己**的出货就绪关卡。强制出货纪律的工具，被同一套纪律
量过，并且通过了。
...
- **绕过钩子的对抗性扫描 —— 在常设 300+ 条语料*之外*新增约 40 种写法**，全部经真实钩子判定:
  ...
  拦截」对照集上过度拦截为 0。**
- **1494 tests green · `tsc` 0 · 重复运行结果确定。**
