...
| 测试套件 | **1526 tests**（65 个文件）—— **发布包中 1504 项通过**（其余是查看仓库内部文档的检查，这些文档不随发布包分发；`git archive` 实测）。2 项墙钟断言在繁忙机器上不给判定，而是记录**「无法测量」与当时的负载** |
...
（1526 tests），并在此之上通过了本工具**自己**的出货就绪关卡。强制出货纪律的工具，被同一套纪律
量过，并且通过了。
...
- **绕过钩子的对抗性扫描 —— 在常设 300+ 条语料*之外*新增约 40 种写法**，全部经真实钩子判定:
  ...
  拦截」对照集上过度拦截为 0。**
- **1526 tests green · `tsc` 0 · 重复运行结果确定。**
