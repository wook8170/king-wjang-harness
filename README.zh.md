...
| 测试套件 | **1236 项通过**（53 个文件）—— 其中 16 项是仅限仓库的检查，在发布包中会被跳过（发布包为 1220 项） |
| 每会话新增上下文 | 启用时 **约 240 令牌**；没有 `.harness/` 的项目为 **0** |
| 运行时依赖 | **1 个**（`yaml`，已打包） |
| 确定性 | 3 次运行判定完全一致 |

**你可以自己重测** —— `npm run bench:hook` 随包提供。它会合成三种形态（真实、损坏、对抗）的
10 万条日志，对真实的钩子进程计时，并同时打印**本机 `node` 启动的下限值** —— 因为任何绝对数字里
很大一部分来自这个下限，而不是本工具。
...
| 键 | 默认值 | 作用 |
|---|---|---|
| `lang` | `en` | 工具自身输出文案的语言 —— CLI、钩子 JSON 与生成文档。韩语为 `ko`。**MCP 的工具说明与拒绝文案仍为英文**（ko 字符串不在 MCP 包内）。 |
...
**v0 —— 核心引擎、关卡以及构建/出货轨道均已实现并实测**（1236 项测试）。但出货就绪审计的判定
