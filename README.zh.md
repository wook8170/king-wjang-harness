...
<sub>钩子 + CLI · 仅追加（append-only）的事件日志作为唯一真相源 · 每次会话上下文约 260 令牌 · 未启用的项目中保持完全静默</sub>
...
| **上下文开销** | 每加一条规则就多一份开销 | **每次会话约 260 令牌，且固定** —— 规则在进程之外运行，再加一条也不会变多 |
...
>
> `king-wjang-harness` 驱动技能会**检查这些配套技能，并安装缺失的**——这是一次网络操作，经由常规 Bash 权限层，且仅限已知的市场来源。它并非必需：即便没有它们，钩子照样强制流程。
...
### Claude Design 集成 —— 画布获取由 P4 技能驱动

*（本节标题原为「路线图」。它以安静的方式出错了 —— 下列全部均已实现并实测。画布获取不在核心中 —— 这是为保持强制钩子本地化的«有意»设计 —— 而是由 P4 技能用 WebFetch 取回并把正文交给核心。过小宣传同样是文档缺陷，只是没有人会抱怨。）*

- **画布（Canvas）= 可视化的唯一真相源。** 一块画板 ↔ 一个 UX 节点（`"UX-7 Checkout"`）；画布 URL 保存在设计台账里。 **核心**不会通过网络获取画布（以保持强制钩子的本地与快速）。改由 **P4 技能**代劳：用 WebFetch 取回画布，并将其正文以一个流程交给 `harness design sync <UX-x> --from <文件>`——由智能体拉取画布，核心保持本地。
...
- **运行时自包含** —— 构建产物 `core/dist/` 已被提交；直接克隆即可使用，无需任何构建步骤。`yaml` 被内联打包在内（其 ISC 声明随打包产物一同分发），运行时入口的外部引用只有 Node 内置模块。`npm audit --omit=dev`：**0 个漏洞。**（范围说明：`claude plugin install` 会在插件缓存目录内执行 `npm ci --ignore-scripts`。本仓库随包提供写有 `omit=dev` 的 `.npmrc`，因此开发工具链不再落盘 —— 实测每个版本 **1.3 MB、10 个包**（此前约 81 MB）。若要自行运行发布包内的测试，用 `npm install --include=dev` 显式开启。）
...
| 测试套件 | **1596 tests**（68 个文件）—— **发布包中 1572 项通过**（其余是查看仓库内部文档的检查，这些文档不随发布包分发；`git archive` 实测）。2 项墙钟断言在繁忙机器上不给判定，而是记录**「无法测量」与当时的负载** |
| 每会话新增上下文 | 启用时 **约 260 令牌**；没有 `.harness/` 的项目为 **0**。复测: `echo '{"hook_event_name":"SessionStart"}' \| ./bin/harness-hook session-start \| wc -c` —— 注入的简报约 1KB（其中包含项目路径，故实际大小略有出入），除以 4 即为令牌估算 |
| 运行时依赖 | **1 个**（`yaml`，已打包） |
| 确定性 | 3 次运行判定完全一致 |
...
```bash
npm install --include=dev   # .npmrc 会排除 dev 依赖，故显式开启。prepare hook 构建 core/dist
./bin/harness --version
```
</details>
...
**`harness` 在哪里。** CLI 随插件一起分发。Claude Code 会把插件的 `bin/` 加入**智能体执行命令所在
shell 的 PATH**，所以在那个 shell 里下表可以照写照用 —— 这些命令通常也正是在那里执行的。你自己的
终端不会获得该 PATH 条目；若要手动运行，请自行加入已安装插件的 `bin/`：
...
| `harness gate submit <P0..P12>` | 将该阶段的产物提交评审 |
| `harness gate approve <P0..P12>` | **仅限人工** —— 打开下一阶段的批准（不要加入允许列表） |
| `harness report denials [--limit <n>]` | 钩子最近拒绝的工具调用 —— 什么被拦下、为什么 |
| `harness loop critical raise --reason <r> [--detail <d>]` · `harness loop critical clear` | 呼叫人工并停止 wave 循环；判断完成后解除 |

`harness --help` 会打印命令地图，`harness <命令组> --help` 会列出该组的子命令；这张表是简要参考。钩子事件（`harness hook …`）由插件调用，从不需要手动调用。

**如何卸下。** 没有 `harness uninstall`，这是有意为之：智能体不应能够关闭它自己正在受约束的规则。
解除是人在自己终端里做的动作。

```bash
rm -rf .harness/          # 只针对这个项目 —— 钩子会立即安静下来
```

事件日志也会一并删除。日志是审计轨迹，若要保留请先拷贝出来。
若要在所有项目中停用，请卸载插件（`claude plugin uninstall king-wjang-harness`）。
...
（1596 tests），并在此之上通过了本工具**自己**的出货就绪关卡。强制出货纪律的工具，被同一套纪律
量过，并且通过了。
...
- **13 个出货关卡 → 12 个实测 PASS**（第 13 个钩子延迟在空闲机器上可干净复测）:
  ...
  批准）· 确定性 · 供应链（`npm audit --omit=dev`: **0**）· 密钥历史（这是 gitleaks 规则能捕获的范围，与 GitHub 推送保护拦截的范围并不相同；本次审计中两者对同一字符串给出了不同判定）（gitleaks，307 个提交:
  **0**）· CLI 契约 · 可观测性 · 打包。
- **绕过钩子的对抗性扫描 —— 在常设 300+ 条语料*之外*新增约 40 种写法**，全部经真实钩子判定:
  ...
  拦截」对照集上过度拦截为 0。**
- **1596 tests green · `tsc` 0 · 重复运行结果确定。**
...
- ✅ **阶段技能 —— 全部十三个阶段**（P0–P12：设计 `P0–P6`、构建 `P7–P9`、出货 `P10–P12`）+ researcher / design-auditor / wave-executor / wave-verifier / readiness-auditor 智能体 + ADR（`adr`）
...
- **以«文件»形式交给解释器的程序，读取方式与 shell 脚本相同** —— 当程序作为文件传给解释器时（`sed -f prog.sed`、`awk -f prog.awk`、`perl x.pl`、`python3 x.py`、`node x.js`、`bun`、`deno run`、`ruby`、`php`、`tclsh`、`lua`、`Rscript`），钩子会读取该文件，若它写入 harness 管辖的路径就拒绝。**三个边界是有意为之的**：① 程序文件**超过 64 KB 时是跳过而非拒绝**（真实的打包产物很大，而手写的伪造文件很小 —— 超过 64 KB 的伪造文件，是为了不阻塞 `node dist/cli.js` 而公开承担的代价）；② **钩子不认识的解释器**（如 `julia`、`groovy`、`raku` 等冷门运行时）不在枚举范围内；③ **不把路径写成字面量、而在语言内部拼装的程序** —— 字符串拼接（`".har" + "ness/…"`）、`chr()`/`String.fromCharCode` 或 base64 —— 不会被字面量路径检查捕获（进入受保护目录的相对 `chdir(".harness")` 会被捕获）。要完全关闭解释器长尾与语言内混淆，需要的是**文件系统层的强制**，而不是钩子读取程序体；那超出了钩子的范围。
- **有意不提供日志压缩命令。** `events.jsonl` 是审计记录，重写它的命令等于在**唯一不允许删除的地方**提供删除原语。没有它的代价是有界的 —— 10 万条 / 15 MB（数十年用量）重放相对正常路径多加 p95 +17.7 ms（进程内；挂钟时间 +24.7 ms），而且只在状态降级期间发生，`doctor --repair` 即可结束。**v0.2 增加的是上限，而不是压缩**：日志超过 128 MB、设计台账与指令书超过 16 MB 时会拒绝读取，并给出「请自行归档」的处方。而且 `doctor` 会在到达上限一半时提前告警 —— 极限在触及之前就可见，同时仍然没有任何东西替你删除审计记录。
