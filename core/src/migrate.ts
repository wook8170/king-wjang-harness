...
const CANDIDATES: ReadonlyArray<{
...
}> = [
  {
    ...
    type: 'dir',
    action: '하네스가 Stop 차단·SessionStart 주입을 대체한다(웨이브 상태 기반 정확 판정). settings.json 의 handoff-guard 훅 등록을 지워라 — 두 훅이 같은 턴을 이중으로 막는다.',
  },
  {
    name: 'token-guard',
    kind: 'hook',
    ...
    type: 'dir',
    action: '하네스가 사용량 티어 판정·지침 주입을 대체한다. settings.json 의 PostToolUse·UserPromptSubmit 훅 등록을 지워라 — 같은 티어 문구가 두 번 주입된다.',
  },
  {
    name: 'auto-retry',
    kind: 'job',
 ...
 * 사용자가 diff 로 읽는다).
 */
export function detectLegacyTools(homeDir: string): LegacyTool[] {
...
export function migrationReport(tools: LegacyTool[]): string {
  if (tools.length === 0) {
