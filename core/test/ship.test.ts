...
const approveShipGate = (root: string, phase: 'P10' | 'P11', rel: string) => {
  ...
  // 페이즈 이름만 다른 사본은 쓰지 않는다 — 거의 같은 문서로 다른 게이트를 여는 것은
  // SEC-79 계열로 막혀 있고, 픽스처가 그 모습이면 규칙이 아니라 픽스처가 거짓말을 한다.
  const TOPIC = {
    P10: '위협 모델과 그것을 막는 가드, 그리고 실제로 시도해 본 우회 경로를 담는다. '
      + '심링크·접두 명령·래퍼를 씌운 호출까지 돌려 보고, 막힌 것과 막지 말아야 할 것을 '
      + '짝으로 기록한다. 과차단이 나오면 그것도 결함으로 센다.',
    P11: '배포 경로와 되돌리는 절차, 그리고 배포 후 무엇을 관측하는지를 담는다. '
      + '카나리 구간의 임계값과 그 임계를 그렇게 잡은 근거, 실패 시 자동으로 되돌아가는 '
      + '조건을 적는다. 배포 기록은 저널에 남는다.',
  } as const;
  writeDoc(root, rel, `# ${phase} 산출물\n\n${phase} 트랙의 심사 대상이다. ${TOPIC[phase]}\n`);
