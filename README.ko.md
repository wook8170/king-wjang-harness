...
| 테스트 스위트 | **1282 passing** (53 파일) — 그중 16건은 리포 전용 검사라 배포본에서는 skip 된다(배포본 1266건) |
| 세션당 추가 컨텍스트 | 하네스가 켜졌을 때 **~240 토큰** · `.harness/` 없는 프로젝트는 **0** |
| 런타임 의존성 | **1** (`yaml`, 번들) |
| 결정성 | 3× 실행 동일 판정 |

**직접 다시 재 보라** — `npm run bench:hook` 이 패키지에 함께 실린다. 10만 건 저널을 세 부류
(현실·손상·적대)로 합성해 실제 훅 프로세스를 재고, **이 머신의 `node` 기동 바닥값**을 함께 찍는다 —
...
**v0.1.0 — 코어 엔진·게이트·구축/출하 트랙까지 구현·실측 완료** (1282 tests). 다만 출하 검증 판정은
...
- **저널 압축 명령은 의도적으로 두지 않는다.** `events.jsonl` 은 감사 기록이라 그것을 다시 쓰는 명령은 **아무것도 지워서는 안 되는 자리의 삭제 원시연산**이 된다. 없어서 치르는 비용은 한계가 분명하다 — 10만 건·15 MB(수십 년 분) 재생 p95 ≈ 19 ms(인프로세스, wall-time 기준 +29 ms)이고, 그나마 상태가 열화된 동안만이며 `doctor --repair` 로 끝난다.
