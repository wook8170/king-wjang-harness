...
- **런타임 자체완결** — 빌드본 `core/dist/`가 커밋돼 있어 순수 클론이 빌드 단계 없이 동작한다. `yaml`은 인라인 번들이고, 런타임 진입점의 외부 참조는 Node 내장뿐이다. `npm audit --omit=dev`: **취약점 0.** (범위: `claude plugin install` 은 플러그인 캐시에서 `npm ci --ignore-scripts` 를 돌므로 개발 툴체인이 그곳 디스크에는 내려온다 — 버전당 73패키지·약 66MB. 그중 어느 것도 실행 경로에 없다. CLI·훅·MCP 서버는 `node_modules` 가 없어도 그대로 동작한다.)
...
| 테스트 스위트 | **1494 passing** (63 파일) — 그중 20건은 리포 전용 검사라 배포본에서는 돌지 않는다(**배포본 1474건**, `git archive` 실측) |
...
(1494 tests), 그 위에 하네스 **자신의** 출하 준비 게이트를 통과했다. 출하 규율을 강제하는 도구를
그 규율로 재어 통과시켰다.
...
- **1494 tests green · `tsc` 0 · 반복 실행에서 결정적.**
