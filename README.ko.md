...
| 테스트 스위트 | **1257 passing** (53 파일) — 그중 16건은 리포 전용 검사라 배포본에서는 skip 된다(배포본 1241건) |
...
**v0 — 코어 엔진·게이트·구축/출하 트랙까지 구현·실측 완료** (1257 tests). 다만 출하 검증 판정은
...
**버그 신고.** 이 플러그인에는 아직 공개 이슈 트래커가 없다 — 설치해 온 저장소가 곧 신고
경로다. `harness doctor` 출력과 `harness --version` 을 함께 적어라(둘 다 파일 내용을 담지
않으므로 그대로 붙여도 안전하다).
