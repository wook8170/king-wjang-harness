...
| 테스트 스위트 | **1277 passing** (53 파일) — 그중 16건은 리포 전용 검사라 배포본에서는 skip 된다(배포본 1261건) |
...
| 키 | 기본값 | 하는 일 |
|---|---|---|
| `lang` | `en` | 하네스 자신이 내는 문구의 언어 — CLI·훅 JSON·생성 문서. 한국어는 `ko`, 한 번만 바꾸려면 `HARNESS_LANG=ko`. **MCP 도구 설명·거절문은 영어로 남는다**(ko 문자열이 MCP 번들에 없다). |
...
**v0.1.0 — 코어 엔진·게이트·구축/출하 트랙까지 구현·실측 완료** (1277 tests). 다만 출하 검증 판정은
