...
| 테스트 스위트 | **1548 tests** (66 파일) — **배포본에서 1524건 통과**(나머지는 배포본에서 빠지는 리포 내부 문서를 보는 검사다, `git archive` 실측). 시간 단언 2건은 바쁜 머신에서 판정 대신 **「측정 불가」와 그때의 부하**를 적는다 |
...
(1548 tests), 그 위에 하네스 **자신의** 출하 준비 게이트를 통과했다. 출하 규율을 강제하는 도구를
그 규율로 재어 통과시켰다.
...
- **1548 tests green · `tsc` 0 · 반복 실행에서 결정적.**
