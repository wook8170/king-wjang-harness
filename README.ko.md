...
| 테스트 스위트 | **1404 passing** (58 파일) — 그중 17건은 리포 전용 검사라 배포본에서는 skip 된다(배포본 1364건) |
...
(1404 tests), 그 위에 하네스 **자신의** 출하 준비 게이트를 통과했다. 출하 규율을 강제하는 도구를
그 규율로 재어 통과시켰다.
...
- **1404 tests green · `tsc` 0 · 반복 실행에서 결정적.**
