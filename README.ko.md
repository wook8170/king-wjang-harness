...
| 테스트 스위트 | **1324 passing** (54 파일) — 그중 17건은 리포 전용 검사라 배포본에서는 skip 된다(배포본 1307건) |
...
**v0.1.0 — 코어 엔진·게이트·구축/출하 트랙까지 구현·실측 완료** (1324 tests). 다만 출하 검증 판정은
...
- **밖에서 들어온 내용은 이미 존재하는 디렉토리 아래로 별칭을 들여올 수 있다.** 전개·클론·패키지 설치 뒤 **새로 생길 경로**에 쓰는 것은 거부하지만, **이미 있는 디렉토리** 안에 쓰는 것은 막지 않는다 — 그것이 `npm install` 이 정상적으로 하는 일이기 때문이다. 이것을 완전히 닫는 일은 훅이 아니라 파일시스템 층의 몫이다.
