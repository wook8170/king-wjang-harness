...
- **Interpreter program files are read the same way a shell script is** — when a program is handed to an interpreter as a *file* (`sed -f prog.sed`, `awk -f prog.awk`, `perl x.pl`, `python3 x.py`, `node x.js`, `bun`, `deno run`, `ruby`, `php`, `tclsh`, `lua`, `Rscript`), the hook reads that file and denies it if it writes a harness-owned path. Two bounds are deliberate: a program file **over 64 KB is skipped, not denied** (real bundles are large and hand-written forgers are tiny — a >64 KB forger is the disclosed cost of not blocking `node dist/cli.js`), and an **interpreter the hook does not know** (exotic runtimes such as `julia`, `groovy`, `raku`) falls outside the enumerated set. Closing the interpreter long-tail completely needs filesystem-layer enforcement, not hook enumeration; that is out of scope for the hook.
