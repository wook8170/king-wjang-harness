...
이 리포 자신의 다음 빌드 작업에 먼저 적용해 검증 후 제품 스킬로 확정. 단 §6.0 전제조건(`.harness/` gitignore) 충족 필요, 그리고 이 리포는 현재 설계/보안 트랙이라 실제 병렬 P8 dogfood 대상 웨이브가 마땅치 않을 수 있음 → dogfood 대상 별도 협의(작은 독립 개선 2~3건을 인위적 병렬 라운드로). **워크트리 비용 주의**(v2: MINOR-1): 리포 안 `.claude/worktrees/` N체크아웃 → 메인의 전역 glob(테스트·린트·grep)이 사본을 주울 수 있고(러너 exclude 확인 필요), next-app 이면 워크트리마다 `npm install`(분·GB × N)이 처리량 이득을 상쇄 → pnpm/캐시 공유 또는 워크트리 재사용.

## 9. 위험 · 미해결

- **`.harness/` 스탠스 긴장**(BLOCKER-1): 코어 주석(커밋)과 오케스트레이션 요구(gitignore)가 반대 — §1.1 에서 병렬은 gitignore 로 못박되, 하네스 전체 공식 스탠스 결정은 열린 항목.
