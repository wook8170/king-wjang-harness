...
> **개정 [FEAT-73] — 「첫 행동으로 지시」를 「선택 안내」로 내린다.**
> 위 본문은 이 플러그인이 `/remote-control` 을 제공한다는 전제 위에서만 옳았다. 측정하면
> `commands/` 디렉토리도 `plugin.json` 의 commands 키도 **없다** — 그 명령은 클라이언트 내장이거나
> 다른 곳에서 오고, 훅은 있는지 알 수 없다(슬래시 명령 목록을 훅이 조회할 방법이 없어
> 파일시스템 탐침은 거짓 음성을 낳는다). 그래서 **없는 환경에서는 매 세션의 첫 행동이 실패**하고,
> 하네스가 실제로 보장하는 일(활성 웨이브 이어받기·정산)이 뒤로 밀렸다.
> 구현은 다음으로 바꾼다 — 기본값 `on` 과 요구 15 자체는 유지한다:
> 1. 번호 붙은 지시 목록에는 **하네스가 보장하는 것만** 넣는다(첫 지시 = 활성 웨이브 이어받기).
> 2. Remote Control 은 주입 **맨 뒤의 조건부 안내** 한 줄 — 「있으면 실행, 없으면 건너뛴다」 +
>    위 열화 경로(푸시·아티팩트)를 그 자리에서 명시 + 끄는 법(`remote_control: false`) 안내.
> 3. `remote_control: false` 는 언급 자체를 없앤다(옵트아웃 불변).
> 기본값을 `off` 로 내리지 **않은** 이유: 있는 환경에서는 실제로 동작하는 채널이고,
> 훅은 어느 쪽인지 모른다 — 모르는 것을 이유로 기능을 끄면 요구 15가 조용히 사라진다.
