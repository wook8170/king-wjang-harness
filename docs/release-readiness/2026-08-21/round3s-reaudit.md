...
- **후속 (2026-08-27, 이 감사 밖)**: 정본 대장 `ledger.md` 에 `FLAKE-01` **등재 완료**, 두 단언을 부하 둔감형으로 **수정·봉인**했다 — COST-129 는 같은 실행 안의 비율(캐시 100배 이상 빠를 것, 실측 여유 6673배), COST-228 은 최소값 3회 + 부류 문턱 2000ms(유휴 4~51ms, 수정 전 회귀 50KB 3495ms). 변이 검증으로 둘 다 «깨지면 red» 확인.
