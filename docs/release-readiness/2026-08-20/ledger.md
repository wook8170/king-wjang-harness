...
|---|---|---|---|---|---|---|---|
| SEC-01 | MED | 06 | 보호 디렉토리 자체가 외부 심링크면 그 실경로 직접 쓰기가 P8에서 허용 | open | measured | `06-security.md` 임무2 · `core/src/hook.ts` · Bash 표면과 동급이라 기존 수용 표면 이내 | — |
| SEC-02 | LOW | 06 | realpath 판정의 TOCTOU 잔존(판정 후 심링크 교체) — 계약상 "사고 방지 장치" 범위 | deferred | code | `core/src/hook.ts` 헤더 계약 | 문서화로 갈음(사고 방지 장치 계약 범위) — 로드맵 후속 |
...
| SHIP-02 | MED | 10 | 이 커밋 이전에 init된 기존 `.harness/`의 `*`-only `.runtime/.gitignore` 마이그레이션 부재 | open | measured | `10-deploy.md` §4 — 신버전 실행 후에도 `*\n` 불변·`git ls-files` 빈 출력(클론서 .runtime 유실→관측채널 소실) · doctor 검사 후보 | — |
...
| API-01 | LOW | 02 | `--refs` 원장 검증이 CLI 전용 — `createWave()` 직접 호출은 우회 | deferred | code | `core/src/cli.ts` | v0 수용(CLI가 유일 소비자) — 게이트 CLI(로드맵 2) 때 재고 |
| API-02 | LOW | 02 | `MultiEdit`은 현행 도구 목록에 없는 죽은 분기 (무해) | deferred | code | `core/src/hook.ts` WRITE_TOOLS | 하위호환 유지 결정(이전 세션) — 무해 |
| LOGIC-01 | MED | 08 | 훅 자기호출 식별이 정규식(I6) — CLI 마커 방식 근본책은 로드맵 후속 | deferred | code | `core/src/hook.ts` HARNESS_CMD_RE | 명령 위치 한정으로 완화됨 — 근본책은 로드맵 후속 |
| LOGIC-02 | LOW | 08 | doctor 정산 후 웨이브 파일이 복귀하면 frontmatter status:active 잔존(재활성화 가능) | open | code | `core/src/doctor.ts`·`core/src/wave.ts` — doctor는 웨이브 frontmatter를 재기록하지 않음 | — |
| OPS-01 | LOW | 11 | replayState가 「버린 이벤트 수」를 반환하지 않음(M1) | deferred | code | `core/src/events.ts` | 로드맵 후속 — 관측 보강 |
| OPS-02 | MED | 11 | doctor에 웨이브/원장 정합 검사 부재(M3) | deferred | code | `core/src/doctor.ts` · OPS-10이 실증(doctor 손상 웨이브 맹점) | 로드맵 후속 — 정합 검사 추가 |
...
| OPS-11 | LOW | 11 | `harness status`가 미초기화 state.json에 raw ENOENT 노출(손상 파싱에러는 OPS-16로 분리) | verified | measured | `11-ops.md` · `core/src/cli.ts` | `2efe05d` 미초기화 status는 init 안내(ENOENT). 손상 state.json 파싱에러 안내는 OPS-16로 이월 |
| OPS-16 | LOW | 11 | `harness status`가 손상 state.json(유효치 않은 JSON)에 raw 파싱에러 노출·doctor 안내 없음 (미초기화 케이스는 OPS-11로 해소) | deferred | measured | `core/src/cli.ts` status 분기 · 재현: state.json=`NOTJSON{{{`→raw 파싱에러 | 출하 후 백로그 — doctor --repair 안내 추가 후보 |
...
| LOGIC-12 | MED | 08 | (=API-10 중복) bumpNode 파서 이중화로 스칼라 참조 부분문자열 오탐 STALE | verified | measured | `08-logic.md` LOGIC-12 = `02-backend.md` API-10 | `2efe05d`로 함께 해소(F2리뷰 Approved) |
