...
**갱신** 2026-08-20(전 10축 감사 완료·측정 게이트 실측) · **판정** 조건부 출하 가능 ·
**open BLOCKER** 0 · **open HIGH** 3(LOGIC-10·LOGIC-11·SHIP-11) · **open MED** ~11 ·
**open LOW** ~15 · **deferred** 4

측정 게이트 전건 PASS(조용한 창): G1 테스트 171 pass×3 동일 · G2 tsc 0 · G3 빌드+version ·
G4 공급망 프로덕션도달 0 · G5 이력비밀 0 · G6 훅무해 17/17 · G7 페르소나 P1~P4 실패0 ·
G9 훅지연 p95 59ms · G10 doctor/훅 60ms대 · G11 신규설치·G12 업그레이드/롤백 성공.
남은 것은 게이트가 아니라 **HIGH 3건(조건)**: 아래 상세.
...
| USE-01 | LOW | 04 | `wave activate <없는 id>` raw ENOENT 노출 (활성 잠금 케이스는 e48473d로 해소) | open | measured | `04-usability.md` · 재현: `harness wave activate wave-999` → ENOENT | — |
...
| SHIP-11 | HIGH | 10 | `core/dist`가 gitignore(미커밋)+플러그인 설치에 빌드 단계 없음 → 순수 클론 설치 시 하네스 무동작(inert): 전 훅이 빌드본 부재 가드로 no-op(무해하나 무기능) | open | measured | `10-deploy.md` SHIP-11 재현 — `git archive`→dist부재→`rm -rf /` Bash도 무평가 exit0 · 해소: dist 커밋 or 설치 빌드단계 문서화 | — |
| SHIP-12 | MED | 10 | 사용자용 설치 문서(README/INSTALL) 부재 — 동작하는 절차(`npm install && npm run build`)가 내부 플랜 문서에만 존재 | open | measured | `10-deploy.md` §1 · 저장소 전수 README/INSTALL 0개 | — |
...
| OPS-11 | LOW | 11 | `harness status`가 손상 state.json에 raw JSON 파싱 에러로 사망, doctor 안내 없음 | open | measured | `11-ops.md` · cli.ts:101-103 · 재현: state.json=`NOTJSON{{{`→`Unexpected token 'N'...`·exit 1 | — |
...
| API-10 | MED | 02 | STALE 전파 규칙 복제 — bumpNode가 parseWave 미재사용, 스칼라 `design_refs: UX-10`에서 `.includes('UX-1')` 부분문자열 오마킹 | open | measured | `02-backend.md` API-10 · ledger.ts:67-79 vs wave.ts:15-33 · 재현: bump UX-1이 UX-10 참조 웨이브 오마킹 | — |
| API-11 | LOW | 02 | JSON `null` stdin이 fail-open TypeError로 기록돼 doctor 훅경보 오염(무해는 유지, 42/"str"/[]는 무증상=비일관) | open | measured | `02-backend.md` API-11 · cli.ts:78 · `printf null \| hook pre-tool`→exit0+로그 | — |
| API-12 | LOW | 02 | raw ENOENT 절대경로 유출 — ENOENT 안내화가 정본(readActiveWave)에 있으나 status·activate 미적용(재사용 누락, USE-01과 동근) | open | measured | `02-backend.md` API-12 · wave.ts:168-179 vs 미적용 | — |
...
| SEC-10 | MED | 06 | frontmatter(milestone/design_refs/acceptance)·backtrack.reason이 펜스·절단 없이 session-start 지시 채널로 주입 → 위조 `지시(N):` (턴로그 방어가 형제 필드 미적용) | open | measured | `06-security.md` SEC-10 · hook.ts:154-161,177-178 | — |
| SEC-11 | LOW | 06 | 턴로그 데이터펜스가 정적 구분자라 본문이 `--- 발췌 끝 ---` 재현 시 breakout | open | measured | `06-security.md` SEC-11 · hook.ts:47-49 · nonce 구분자 권고 | — |
| SEC-12 | LOW | 06 | "루트 밖" deny 사유가 raw file_path 무절단 반향(개행·ANSI 포함) | open | measured | `06-security.md` SEC-12 · hook.ts:310 | — |
| SEC-13 | LOW | 06 | hook.ts logHookError가 runtimeDir mkdir 안 함 → 신규 클론(.runtime 부재)서 훅 fail-open 침묵(SHIP-02·OPS-12 교차) | open | measured | `06-security.md` SEC-13 · hook.ts:94-103 vs cli.ts:44 · state.ts:44 자기명시 | — |
...
| LOGIC-10 | HIGH | 08 | state.json 형태 손상(유효 JSON `{}`/`[]`/`"hello"`) 시 저널 폴백 미발동 → phase=undefined → 설계트랙 소스 차단·stop 가드 침묵 해제, 무흔적 | open | measured | `08-logic.md` LOGIC-10 · hook.ts:59-69 · 재현: `echo {} > state.json`→P0 src Write allow | — |
| LOGIC-11 | HIGH | 08 | state.json 삭제 시 훅 전면 침묵(events.jsonl·활성웨이브 잔존해도) — isInitialized(state.json) vs harnessDir 정의 이원화, 무흔적 | open | measured | `08-logic.md` LOGIC-11 · hook.ts:58, state.ts:19-21 · 재현: 활동 후 `rm state.json`→훅3종 무응답 | — |
| LOGIC-12 | MED | 08 | (=API-10 중복) bumpNode 파서 이중화로 스칼라 참조 부분문자열 오탐 STALE | open | measured | `08-logic.md` LOGIC-12 = `02-backend.md` API-10 · 단일 수정으로 함께 해소 | — |
...
| LOGIC-16 | LOW | 08 | `node upsert --status`가 열거형 무검증 — draft/approved/stale 밖 값 원장 기록(frontmatter는 정규화하는데 CLI는 캐스트만) | open | measured | `08-logic.md` LOGIC-16 · cli.ts:176 vs wave.ts:29 | — |
